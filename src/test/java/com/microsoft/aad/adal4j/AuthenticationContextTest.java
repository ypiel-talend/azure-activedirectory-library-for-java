...
@Test(groups = { "checkin" })
@PrepareForTest({ AuthenticationContext.class, AuthenticationCallback.class, AsymmetricKeyCredential.class })
public class AuthenticationContextTest extends AbstractAdalTests {
	...
	public void testAcquireTokenAuthCode_ClientCredential() throws Exception {
		ctx = PowerMock.createPartialMock(AuthenticationContext.class, new String[] { "acquireTokenCommon" },
				TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		PowerMock
				.expectPrivate(ctx, "acquireTokenCommon", EasyMock.isA(AdalAuthorizatonGrant.class),
						EasyMock.isA(ClientAuthentication.class), EasyMock.isA(ClientDataHttpHeaders.class))
				.andReturn(new AuthenticationResult("bearer", "accessToken", "refreshToken", new Date().getTime(),
						"id_token", new Date().getTime(), null, false));
		PowerMock.replay(ctx);
		Future<AuthenticationResult> result = ctx.acquireTokenByAuthorizationCode("auth_code",
				new URI(TestConfiguration.AAD_DEFAULT_REDIRECT_URI), new ClientCredential("clientId", "clientSecret"),
				new String[] { "scope" }, null);
	...
	public void testcAquireTokenAuthCode_AuthCodeNull() throws Exception {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireTokenByAuthorizationCode(null, new URI(TestConfiguration.AAD_DEFAULT_REDIRECT_URI),
				new ClientCredential("clientId", "clientSecret"), new String[] { "scope" }, null);
	}
	...
	public void testAcquireTokenAuthCode_RedirectUriNull() throws Exception {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireTokenByAuthorizationCode("auth_code", null, new ClientCredential("clientId", "clientSecret"),
				new String[] { "scope" }, null);
	}
	...
	public void testAcquireTokenAuthCode_KeyCredential() throws Exception {
		...
		PowerMock
				.expectPrivate(ctx, "acquireTokenCommon", EasyMock.isA(AdalAuthorizatonGrant.class),
						EasyMock.isA(ClientAuthentication.class), EasyMock.isA(ClientDataHttpHeaders.class))
				.andReturn(new AuthenticationResult("bearer", "accessToken", "refreshToken", new Date().getTime(),
						"idtoken", new Date().getTime(), null, false));
		final KeyStore keystore = KeyStore.getInstance("PKCS12", "SunJSSE");
		keystore.load(
				new FileInputStream(this.getClass().getResource(TestConfiguration.AAD_CERTIFICATE_PATH).getFile()),
		...
		Future<AuthenticationResult> result = ctx.acquireTokenByAuthorizationCode("auth_code",
				...
				AsymmetricKeyCredential.create(TestConfiguration.AAD_CLIENT_ID, key, cert), new String[] { "scope" },
				null);
		AuthenticationResult ar = result.get();
		Assert.assertNotNull(ar);
		PowerMock.verifyAll();
	...
	public void testAcquireToken_KeyCred() throws Exception {
		...
		final X509Certificate cert = (X509Certificate) keystore.getCertificate(alias);
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		final Future<AuthenticationResult> result = ctx.acquireToken(TestConfiguration.AAD_SCOPE,
				AsymmetricKeyCredential.create(TestConfiguration.AAD_CLIENT_ID, key, cert), null);
	...
	public void testInvalidClientAssertion() throws MalformedURLException {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireToken(TestConfiguration.AAD_SCOPE, new ClientAssertion("invalid_assertion", "clientId"), null);
	}
	...
	public void testValidateInput_ValidateNullResource() throws MalformedURLException {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireToken(null, new ClientAssertion("invalid_assertion", "clientId"), null);
	}
	...
	public void testValidateInput_NullCredential() throws MalformedURLException {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireToken(TestConfiguration.AAD_SCOPE, (ClientAssertion) null, null);
	}

	@Test(expectedExceptions = IllegalArgumentException.class, expectedExceptionsMessageRegExp = "refreshToken is null or empty")
	public void testValidateRefreshTokenRequestInput_NullRefreshToken() throws MalformedURLException {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireTokenByRefreshToken(null, "client_id", new ClientAssertion("invalid_assertion", "clientId"),
				new String[] { "scope" }, null);
	}
	...
	public void testValidateRefreshTokenRequestInput_NullClientId() throws MalformedURLException {
		ctx = new AuthenticationContext(TestConfiguration.AAD_TENANT_ENDPOINT, true, service);
		ctx.acquireTokenByRefreshToken("refresh_token", null, new ClientAssertion("invalid_assertion", "clientId"),
				new String[] { "scope" }, null);
	}
	...
	public void testFailedAcquireTokenRequest() throws Throwable {
		...
		Future<AuthenticationResult> result = ctx.acquireTokenByRefreshToken("refresh",
				new ClientCredential("clientId", "clientSecret"), new String[] { "scope" }, null);
		try {
			result.get();
	...
	public void testFailedAcquireTokenRequest_ExecuteCallback() throws Throwable {
		...
		PowerMock.replay(ac);
		Future<AuthenticationResult> result = ctx.acquireTokenByRefreshToken("refresh",
				new ClientCredential("clientId", "clientSecret"), new String[] { "scope" }, ac);
		try {
			result.get();
		} catch (ExecutionException ee) {
			throw ee.getCause();
